#In questo file calcolo il numero di abitanti in Italia e in ciascuna regione Veneto sulla base del file platea che si trova su github nella cartella dei vaccini. Da qui ricavo il numero di abitanti aventi un età dai 12 anni in su. Il numero di abitanti è quello che si ricava dall'ultimo aggiornamento del file platea

import pandas as pd
vacciniPlatea= pd.read_csv("https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-statistici-riferimento/popolazione-istat-regione-range.csv", sep=",")

#print (vacciniPlatea.info())
numeroDiAbitantiItaliani=vacciniPlatea["totale_generale"].sum()
vacciniPlateaVeneta=vacciniPlatea[vacciniPlatea["denominazione_regione"]=="Veneto"]
numeroDiAbitantiVeneti=vacciniPlateaVeneta["totale_generale"].sum()
moltiplicatore=numeroDiAbitantiItaliani/numeroDiAbitantiVeneti

#Calcoliamo il moltiplicatore per ogni regione
def moltiplicatoreRegione(regione):
	if regione=="P.A. Trento":
		regione="Trento"
	if regione=="P.A. Bolzano":
		regione="Bolzano"
	vacciniPlateaRegione=vacciniPlatea[vacciniPlatea["denominazione_regione"]==regione]
	numeroDiAbitantiRegione=vacciniPlateaRegione["totale_generale"].sum()
	moltiplicatore=numeroDiAbitantiItaliani/numeroDiAbitantiRegione
	print("Gli italiani sono "+str(moltiplicatore)+" volte gli abitanti di "+str(regione))
	return moltiplicatore

#print("Gli Italiani sono " +str(numeroDiAbitantiItaliani))
#print(" I veneti sono "+str(numeroDiAbitantiVeneti))
#print("Gli Italiani sono "+str(moltiplicatore) +" volte i Veneti")
